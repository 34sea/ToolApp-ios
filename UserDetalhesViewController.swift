...
class UserDetalhesViewController: UIViewController {
    var nameUser: String = ""
    ...
    func layout(){
       ...
       // sairButton.frame = CGRect(x: (largura-((largura-50)/2))-20, y: altura-80, width: (largura-50)/2, height: 40)
        toolAlugadosButton.frame = CGRect(x: (largura-((largura-50)))/2, y: altura-80, width: (largura-50), height: 40)
    ...
    @objc func produtosTela(){
        let tela = ToolAlugadaViewController()
        tela.idUser = idUser
        tela.modalPresentationStyle = .fullScreen
        self.navigationController?.pushViewController(tela, animated: true)
    }
